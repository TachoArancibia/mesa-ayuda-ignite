...
module.exports = function (app) {
    ...
    app.get('/test-dashboard', isUserAllowed, function (req, res) {
        res.locals = { title: 'Dashboard de Prueba' };
        res.render('Dashboard/test-dashboard');
    })
      ...
      // Forms
    app.get('/form-test', isUserAllowed, function (req, res) {
        res.locals = { title: 'Formulario de Requerimiento', tipoRequerimiento: req.body._typeTicket };
            //res.render('Form/form-test');
            res.render('Form/form-test', { 'message': req.flash('message'), 'error': req.flash('error') });
    });
    app.post('/form-test', isUserAllowed, urlencodeParser, function (req, res) {
        res.locals = { title: 'Formulario de Requerimiento', tipoRequerimiento: req.body._typeTicket, subProductTicket: req.body._subProductTicket};
        //res.render('Form/form-test');
        res.render('Form/form-test', { 'message': req.flash('message'), 'error': req.flash('error') });
    });

    //Ir a productos de Microsoft Endpoint Manager
    app.get('/mem-products', isUserAllowed, function (req, res) {
        res.locals = { title: 'Microsoft Endpoint Manager' };
        res.render('Products/microsoft-endpoint-manager-products', { 'message': req.flash('message'), 'error': req.flash('error')})
    })
    ...
    //CLC reporte
    app.get('/reportes', isUserAllowed, function (req, res) {
        res.locals = { title: 'Reportes' };
        res.render('Reportes/CLC-reportes');
    })
      // Tables
    app.get('/requerimientos', isUserAllowed, function (req, res) {
        axios.get('http://localhost:8090/api/ticket')
            .then(result => {
                if (result.status == 200) {
                    res.locals = { title: 'Requerimientos', data: result.data };
                    res.render('Tables/requerimientos-datatable')
                }
            })
            .catch(error => {
                console.error(error);
            })
        //res.locals = { title: 'Requerimientos' };
        //res.render('Tables/requerimientos-datatable');
    });
